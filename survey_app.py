...
def calculate_results(scores):
    ...
    for tribe, score in top_tribes:
        if tribe in ["리더쉽과 희생 영역 (유다)", "영적 전투 영역 (갓)", "전도와 선교 영역 (베냐민)"]:
            strengths.append("리더십과 용기")
        elif tribe in ["열정과 변화 영역 (르우벤)", "열정과 전도 영역 (시므온)", "소통과 전달 영역 (납달리)"]:
            strengths.append("열정과 소통능력")
        elif tribe in ["대외활동 영역 (스불론)", "섬김과 봉사 영역 (아셀)", "영향력 행사 영역 (요셉)"]:
            strengths.append("나눔과 섬김")
        elif tribe in ["성실과 안정 영역 (잇사갈)", "헌신과 순종 영역 (레위)", "안정과 신실 영역 (므낫세)"]:
